...
<main id="primary" class="site-main py-16 bg-slate-50">
	<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
		...
		<div class="bg-white rounded-3xl p-8 md:p-12 shadow-md border border-slate-200">
			...
			?>
				<form action="#" method="POST" class="space-y-4">
					...
					<div>
						...
						<input type="text" name="your-name" required class="w-full border border-slate-200 rounded-xl px-4 py-3 text-base focus:border-brand-primary focus:outline-none" placeholder="Họ và tên của bạn">
					</div>

					<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
						<div>
							<label class="block text-sm font-bold text-slate-700 mb-1">Số điện thoại *</label>
							<input type="tel" name="your-phone" required class="w-full border border-slate-200 rounded-xl px-4 py-3 text-base focus:border-brand-primary focus:outline-none" placeholder="Số điện thoại liên hệ">
						</div>
						<div>
							<label class="block text-sm font-bold text-slate-700 mb-1">Email (Không bắt buộc)</label>
							<input type="email" name="your-email" class="w-full border border-slate-200 rounded-xl px-4 py-3 text-base focus:border-brand-primary focus:outline-none" placeholder="Địa chỉ email">
						</div>
					</div>

					<div>
						<label class="block text-sm font-bold text-slate-700 mb-1">Chương trình bạn quan tâm</label>
						<select name="current_program_id" class="w-full border border-slate-200 rounded-xl px-4 py-3 text-base focus:border-brand-primary focus:outline-none">
					...
					<div>
						...
						<textarea name="your-message" rows="4" class="w-full border border-slate-200 rounded-xl px-4 py-3 text-base focus:border-brand-primary focus:outline-none" placeholder="Nhập thắc mắc hoặc câu hỏi cụ thể..."></textarea>
					</div>

					<div class="pt-4 flex flex-col items-center gap-4">
						<button type="submit" class="w-full bg-[#2563EB] text-white py-3.5 rounded-xl font-bold hover:bg-[#1E40AF] transition-all text-base shadow-md">
							ĐĂNG KÝ TƯ VẤN NGAY
						</button>
