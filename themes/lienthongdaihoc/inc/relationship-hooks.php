...
function ltdh_sync_program_relationships( $post_id ) {
	...
	$old_school_id = get_post_meta( $post_id, '_last_known_school_id', true );

	if ( $school_id ) {
		$school_programs = get_post_meta( $school_id, '_offered_programs', true );
		if ( ! is_array( $school_programs ) ) {
			$school_programs = [];
		}
		if ( ! in_array( $post_id, $school_programs ) ) {
			$school_programs[] = $post_id;
			update_post_meta( $school_id, '_offered_programs', $school_programs );
		}
	...
	if ( $old_school_id && $old_school_id != $school_id ) {
		$old_school_programs = get_post_meta( $old_school_id, '_offered_programs', true );
	...
	// 2. Sync with Major
	$major_id = get_field( 'major_relationship', $post_id );
	$old_major_id = get_post_meta( $post_id, '_last_known_major_id', true );

	if ( $major_id ) {
		$major_programs = get_post_meta( $major_id, '_offered_programs', true );
		if ( ! is_array( $major_programs ) ) {
			$major_programs = [];
		}
		if ( ! in_array( $post_id, $major_programs ) ) {
			$major_programs[] = $post_id;
			update_post_meta( $major_id, '_offered_programs', $major_programs );
		}
	...
	if ( $old_major_id && $old_major_id != $major_id ) {
		$old_major_programs = get_post_meta( $old_major_id, '_offered_programs', true );
